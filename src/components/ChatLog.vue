  ...
  <div class="bg-gray-800 text-white p-6 rounded-lg shadow-lg flex flex-col h-full">
    ...
    <div class="flex-grow overflow-y-auto space-y-4 bg-gray-700 p-4 rounded">
      <div v-for="(log, index) in chatLogs" :key="index" class="text-sm">
        ...
        <span class="font-bold">{{ log.sender }}{{ log.receiver ? ` → ${log.receiver}` : '' }}:</span> {{ log.action_message }}
      </div>
    </div>
  ...
  methods: {
    ...
    async sendMessage() {
      if (this.message.trim()) {
        ...
        const receiver = this.selected?.name || null
        const gameLog = {
        ...
        try {
          ...
          await axios.post('http://localhost:3000/messages',  gameLog )
